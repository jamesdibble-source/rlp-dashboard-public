...
function retailBenchmarkPanel(lots, suburb, state) {
  ...
  const compact = l => ({ id: l.source_id || l.id, address: l.address, project: l.comparability?.project || null,
    ...
    source: l.source_url, classification: l.comparability });
  displayedRetailBenchmark = { suburb, state, generatedAt: R.built,
    ...
    comparables: included.map(compact) };
  ...
  return '<section class="v5-card" style="padding:20px;margin-bottom:24px" aria-label="Greenfield comparables">'
    ...
    + '<p style="font-size:12px;margin:12px 0">' + summary.sampleCount + ' qualifying priced lots from ' + summary.selectedCount + ' identified greenfield candidates in this size band. ' + summary.unclassifiedCount + ' current listings have an unclassified product type.</p>'
    + (summary.unclassifiedCount ? '<p style="font-size:12px;color:#7c4951">Classification is incomplete. This sample may favour the projects checked so far and may not represent the whole suburb.</p>' : '')
